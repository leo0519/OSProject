...
int slave_open(struct inode *inode, struct file *filp)
{
	filp->private_data = kmalloc(PAGE_SIZE * 100, GFP_KERNEL);
	return 0;
}
static long slave_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	int addr_len ;
	unsigned int i;
	size_t len, data_size = 0, cur = 0, rec;
	...
	switch(ioctl_num){
		...
		case slave_IOCTL_MMAP:
			while ((rec = krecv(sockfd_cli, buf, sizeof(buf), 0)) != 0) {
				memcpy(file->private_data + cur, buf, rec);
				cur += rec;
			}
			ret = cur;
			break;
